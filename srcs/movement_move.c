...
/*   Updated: 2024/02/29 17:53:20 by ede-siga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void rot_plus(t_elems *elems) //rename
{
	...
	elems->ray.dir_x = elems->ray.dir_x * cos(0.12) - elems->ray.dir_y * sin(0.12);
	elems->ray.dir_y = back_dir_x * sin(0.12) + elems->ray.dir_y * cos(0.1);
	elems->ray.plane_x = elems->ray.plane_x * cos(0.12) - elems->ray.plane_y * sin(0.12);
	elems->ray.plane_y = back_plane_x * sin(0.12) + elems->ray.plane_y * cos(0.12);  	
}
...
void rot_minus(t_elems *elems) //rename
{
	...
	back_dir_x = elems->ray.dir_x;
	elems->ray.dir_x = elems->ray.dir_x * cos(-0.12) - elems->ray.dir_y * sin(-0.12);
	elems->ray.dir_y = back_dir_x * sin(-0.12) + elems->ray.dir_y * cos(-0.12);
	elems->ray.plane_x = elems->ray.plane_x * cos(-0.12) - elems->ray.plane_y * sin(-0.12);
	elems->ray.plane_y = back_plane_x * sin(-0.12) + elems->ray.plane_y * cos(-0.12);
}

void center_cam(t_elems *elems) //rename
{
	rot_minus(elems);
	rot_plus(elems);
	/*double	back_plane_x;
	...
	back_plane_x = elems->ray.plane_x;
	back_dir_x = elems->ray.dir_x;
	elems->ray.dir_x = elems->ray.dir_x * cos(0) - elems->ray.dir_y * sin(0);
	elems->ray.dir_y = back_dir_x * sin(0) + elems->ray.dir_y * cos(0);
	elems->ray.plane_x = elems->ray.plane_x * cos(0) - elems->ray.plane_y * sin(0);
	elems->ray.plane_y = back_plane_x * sin(0) + elems->ray.plane_y * cos(0);*/
}
